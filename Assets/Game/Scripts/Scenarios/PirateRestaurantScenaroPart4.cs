...
namespace foxRestaurant
{
    public class PirateRestaurantScenaroPart4 : BaseScenario<ListenDialoguesEncounter>
    {
        ...
        protected override async Task StartScenarioTyped(ListenDialoguesEncounter encounter)
        {
            ...
            await lameLarry.Say("Я не знаю, что это именно значит в переводе на нормальный,<pause:0.5> но ничего хорошего.");
            await red.Say("Но дети без ума от Хромого Ларри!");
            await red.Say("Они здесь все разнесут, если увидят тебя в костюме!");
            await red.Say("Я один просто не смогу обслужить столько народу!");
            ...
            await red.Say("Киновечер, говоришь...<pause:1> Придется взять на себя специальное кино-меню с коктейлями.<pause:0.75> Будет трудновато.");
